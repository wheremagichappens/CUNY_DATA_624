...
```{r libraries, echo=T}
# predictive modeling
libraries('mlbench', 'caret','pls', 'naivebayes')
...
## The function 'postResample' can be used to get the test set performance values
postResample(pred = lmPred, obs = testData$y)
```
...
Model 3: 
```{r kj-7.2-3}
naive_bayesModel <- train(x = trainingData$x,
                  y = trainingData$y, 
                  method = 'lm',
                  preProc = 'pca', 
                  tuneLength = 5)
...
naive_bayesPred <- predict(naive_bayesModel, newdata = testData$x) 

## The function 'postResample' can be used to get the test set performance values
postResample(pred = naive_bayesPred, obs = testData$y)
```
