...
From hyperparameter tuning of RF, GBM and Cubist, we confirmed GBM had the lowest RMSE on test set of 0.9133422.
...
cubistOptimal <- cubist(df_train[,-1],df_train[,1], committees = 10, neighbors = 9)

# Make predictions
p1 <- rf %>% predict(df_test)
p2 <- gbmModel %>% predict(df_test)
p3 <- cubistOptimal %>% predict(df_test)
...
`ManufacturingProcess32` is the top predictor and among Biological variable, `BiologicalMaterial12` is the top predictor which is 3rd most importan variable among all. 

For top 10, they are equally distributed - 5 from Manufacturing and the others from Biological. Compare to linear and nonlinear models, we found that `ManufacturingProcess32` was always in top 3 for sure. For tree-based model, we see more of balance between Manufacturing and Biological variables than the other optimal linear and nonlinear models in top 10.
```{r kj-8.7b, echo = F}
#code
imp_var <- varImp(gbmModel)
plot(imp_var, top = 10)
```

>> **(c). Plot the optimal single tree with the distribution of yield in the terminal nodes. Does this view of the data provide additional knowledge about the biological or process predictors and their relationship with yield?**

The optimal recursive partitioning tree shows that `ManufacturingProcess32` is at the top. `BiologicalMaterial12`, which was 3rd most important variable in GBM model, appears in the next split node which shows sort of proves why these variables had higher importance scores, thus highly associated with `Yield`, than most of other variables.
