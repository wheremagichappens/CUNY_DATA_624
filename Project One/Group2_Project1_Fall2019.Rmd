...
Pipe One represents 9 days of water flow rate measurements with multiple samples per hour.  In order to align with hourly readings from Pipe Two, a mean of all Pipe One rates in a given hour was taken and labeled by its 'floor' (i.e. 9 for 9:41, 2 for 2:07, etc.). After aggregating, this yielded 236 observations (spanning nine days) for Pipe One compared with 1000 observations (spanning  41days) from Pipe Two.
...
1. Merge the files and use only 236 observations. 
  ...
  + All forecasts would start November 1 rather than December 3 (the end of the time series).   
2. Merge the files and use the whole set to make predictions.  
  ...
  + We would forecast from the natural ending of the Pipe Two readings.

Because daily periodicity is conceivable for this data, it was important to use a frequency of 24. This entailed numbering by day of year and grooming the time series to start on the 7081 hour (which aligns with October 23 01:00 AM).
...
ws_decomp
```
From the decomposition, there appears to be a seasonal component in agreement with the assessment that there might be a daily flowrate periodicity. Also, as expected, around day 306 - where Pipe One flow rates go silent - there is a downward trend followed by a plateau thereafter. 
...
Here we encounter contradictory estimates: `ndiffs()` suggests a difference of 1, and the augmented dicky fuller test suggests that we are stationary as-is. An `auto.arima()` may provide a reasonable starting place.
...
As the ACF remain wholly above the critical threshold the series will likely require differencing as suggested by the `ndiffs()`. On the other hand, the PACF introduces some ambiguity on the need for differencing.  Following the intial downward trend below the critical threshold, there is a slight spike at 24 suggesting a daily period or season that needs to be accounted for in our forecast.
...
We examined a final ACF of the differenced data to ensure that a second first-order difference was not needed; while we assume $d = 1$, the appropriate value of $q$ is not so clear so we use `auto.arima()` to help iterate up on the best starting place.
...
The original decomposition revelaed very little trend, a lot of seasonality, and a substatial amount of random noise.  This is not considered in the model, and is responsible for the majority of the error in it as white noise is never predictable.
