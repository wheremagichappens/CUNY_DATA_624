...
\begin{quote}
\citet{Juliann}, commented out the previous write\_csv operation and
added in another, let's discuss
\end{quote}
...
\CommentTok{# date <- as.character(seq(as.Date('2010-05-01'), length.out=31, by=1))}
\CommentTok{# ATM_FC <-  cbind("Date"=date, "ATM1"=ATM1_fc$mean, "ATM2"=ATM2_fc$mean,}
                 \CommentTok{# "ATM3"=c(NA,NA,NA,NA),"ATM4"=ATM4_fc$mean) %>% as.data.frame()}

\CommentTok{# Combine the forecasts for the different ATMS}
\NormalTok{ATM_ALL_FC <-}\StringTok{ }\KeywordTok{bind_cols}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(ATM1_fc[}\DecValTok{4}\OperatorTok{:}\DecValTok{6}\NormalTok{]),}
                    \KeywordTok{as.data.frame}\NormalTok{(ATM2_fc[}\DecValTok{4}\OperatorTok{:}\DecValTok{6}\NormalTok{]),}
                    \KeywordTok{as.data.frame}\NormalTok{(ATM3_fc[}\DecValTok{5}\OperatorTok{:}\DecValTok{7}\NormalTok{]),}
                    \KeywordTok{as.data.frame}\NormalTok{(ATM4_fc[}\DecValTok{4}\OperatorTok{:}\DecValTok{6}\NormalTok{])) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{rename}\NormalTok{(}\DataTypeTok{ATM1_mean =} \StringTok{'mean'}\NormalTok{,}
         \DataTypeTok{ATM1_low80CI =} \StringTok{'lower.80.'}\NormalTok{,}
         \DataTypeTok{ATM1_low95CI =} \StringTok{'lower.95.'}\NormalTok{,}
         \DataTypeTok{ATM1_upper80CI =} \StringTok{'upper.80.'}\NormalTok{,}
         \DataTypeTok{ATM1_upper95CI =} \StringTok{'upper.95.'}\NormalTok{,}
         \DataTypeTok{ATM2_mean =} \StringTok{'mean1'}\NormalTok{,}
         \DataTypeTok{ATM2_low80CI =} \StringTok{'lower.80.1'}\NormalTok{,}
         \DataTypeTok{ATM2_low95CI =} \StringTok{'lower.95.1'}\NormalTok{,}
         \DataTypeTok{ATM2_upper80CI =} \StringTok{'upper.80.1'}\NormalTok{,}
         \DataTypeTok{ATM2_upper95CI =} \StringTok{'upper.95.1'}\NormalTok{,}
         \DataTypeTok{ATM3_mean =} \StringTok{'mean2'}\NormalTok{,}
         \DataTypeTok{ATM3_low80CI =} \StringTok{'lower.80.2'}\NormalTok{,}
         \DataTypeTok{ATM3_low95CI =} \StringTok{'lower.95.2'}\NormalTok{,}
         \DataTypeTok{ATM3_upper80CI =} \StringTok{'upper.80.2'}\NormalTok{,}
         \DataTypeTok{ATM3_upper95CI =} \StringTok{'upper.95.2'}\NormalTok{,}
         \DataTypeTok{ATM4_mean =} \StringTok{'mean3'}\NormalTok{,}
         \DataTypeTok{ATM4_low80CI =} \StringTok{'lower.80.3'}\NormalTok{,}
         \DataTypeTok{ATM4_low95CI =} \StringTok{'lower.95.3'}\NormalTok{,}
         \DataTypeTok{ATM4_upper80CI =} \StringTok{'upper.80.3'}\NormalTok{,}
         \DataTypeTok{ATM4_upper95CI =} \StringTok{'upper.95.3'}
\NormalTok{         )}

\CommentTok{# Save output}
\KeywordTok{write.csv}\NormalTok{(ATM_ALL_FC, }\DataTypeTok{file=}\StringTok{"forecasts/ATM_FC.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
