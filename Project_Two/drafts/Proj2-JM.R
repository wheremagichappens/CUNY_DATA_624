...
StudentData <- StudentData %>% filter(complete.cases(PH)) %>% mutate(BrandCode=as.factor(BrandCode))
...
## SVM (RBF)
svm_grid <-  expand.grid(sigma = c(.01, .015, 0.2), C = c(0.75, 1, 1.25))
svm_fit1 <- train(PH~., data=train, method = "svmRadial", preProcess = c("center","scale"), metric="RMSE", tuneGrid = svm_grid, trControl=trC, tuneLength=tl)
svm_fit2 <- train(PH~., data=train, method = "svmRadial", preProcess = c("nzv","zv", "BoxCox", "center","scale"), metric="RMSE", tuneGrid = svm_grid, trControl=trC, tuneLength=tl)
svm_test_pred1 <- predict(svm_fit1, newdata = test); 
svm_test_pred2 <- predict(svm_fit2, newdata = test) 

##  MARS
mars_grid <- expand.grid(degree=1:5, nprune = seq(5, 100, by = 10))
mars_fit1 <- train(PH~., data=train, method = 'earth', tuneGrid = mars_grid, trControl = trC, tuneLength=tl)
mars_fit2 <- train(PH~., data=train, method = 'earth', preProcess = c("BoxCox"), tuneGrid = mars_grid, trControl = trC, tuneLength=tl)
mars_test_pred1 <- predict(mars_fit1, newdata = test); 
mars_test_pred2 <- predict(mars_fit2, newdata = test) 

## eNET
enet_grid <- expand.grid(lambda=c(0,0.01,.1), fraction=seq(0.05,1, length=20))
enet_fit1 <- train(PH~., data=train, method = "enet", preProcess = c("center", "scale"), metric="RMSE", tuneGrid = enet_grid, tuneLength = tl, trControl = trC)
enet_fit2 <- train(PH~., data=train, method = "enet", preProcess = c("nzv","zv", "BoxCox", "center","scale"), metric="RMSE", tuneGrid = enet_grid, trControl=trC, tuneLength=tl)
enet_test_pred1 <- predict(enet_fit1, newdata = test); enet_test_pred2 <- predict(enet_fit2, newdata = test) 
...
MARS_VarImp <- varImp(mars_fit2, scale = T);  
eNET_VarImp <- varImp(enet_fit1, scale = T); 
