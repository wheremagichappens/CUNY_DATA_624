...
libraries('default', 'knitr', 'kableExtra','readxl','sqldf','sjPlot','sjmisc','sjlabelled','MASS','sjPlot','broom')
# Plotting Libraries
...
```{r project2a8, echo=F}
outlierTest(lm1) 
#%>% kable(caption="Linear Model 1 Variance Inflation Numbers")%>% kable_styling(full_width = F, position = "center")%>% row_spec()
```
...
#tab_model(lm2);
#tab_model(lm3)
#sjPlot::sjt.lm(lm1, lm2, lm3)
out1<- tidy(lm1)
out2 <- tidy(lm2)
out3 <- tidy(lm3)

out1%>% kable(caption="Linear Model 1") %>% kable_styling() %>% row_spec();
out2%>% kable(caption="Linear Model 2") %>% kable_styling() %>% row_spec();
out3%>% kable(caption="Linear Model 3") %>% kable_styling() %>% row_spec()
```
...
```{r project2a142, include=FALSE}
fitControl <- trainControl(method = "cv")

nnetGrid <-  expand.grid(size = seq(from = 1, to = 10, by = 1),
                        decay = seq(from = 0.1, to = 0.5, by = 0.1))

nn2 <- train(PH~., data = df_train, method = "nnet", metric = "Rsquared", trControl = fitControl,tuneGrid = nnetGrid,verbose = FALSE,linout = 1)
```
...
```{r project2a18, echo=F}
# Make predictions
p1 <- lm3 %>% predict(df_test)
p2 <- nn2 %>% predict(df_test)
...
# Model performance metrics (Andy's code)
sum_t <- data.frame(
  MODEL = c('LinearModel',
            'NNET',
            'Mars'),
